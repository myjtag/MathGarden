...
function predictImage() {
    ...
    //use opencv to do processing
    let image = cv.imread(canvas);
    //convert it to gray scale
    cv.cvtColor(image, image, cv.COLOR_RGBA2GRAY, 0);
    //make the pixesl above a thershold white
    cv.threshold(image, image, 175, 255, cv.THRESH_BINARY);

    //find the image contour
    let contours = new cv.MatVector();
    let hierarchy = new cv.Mat();
    // You can try more different parameters
    cv.findContours(image, contours, hierarchy, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE);

    //find the contour bounding rectangale
    ...
    //resize the image to be at most 20pixels 
    ...
    // console.log(`top: ${TOP},Bot: ${BOTTOM},Left: ${LEFT},Right: ${RIGHT}`);
    ...
    //convert pixel data from int to float
    pixelValues = Float32Array.from(pixelValues);
    //divide every single pixel value by 255
    pixelValues = pixelValues.map((item)=>{
        return item/255.0;
    });
    //console.log(`scaled array: ${pixelValues}`);
    
    //Create the tensor, so you can make the prediction
    const X = tf.tensor([pixelValues]);
    // console.log(`shape of Tensore: ${X.shape}`);
    // console.log(`data type of Tensore: ${X.dtype}`);
